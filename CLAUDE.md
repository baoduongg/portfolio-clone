...
**Per-target namespacing.** Every cloned URL gets a `<site-key>` (origin slug + 8-char SHA-256 hash) and `<page-key>` (pathname slug + 8-char SHA-256 hash), and every artifact for that target lives under the matching namespaced path:
...
## Cloned site: worawork-vercel-app-13882cbe (currently mounted at `/`)

Full research notes: `docs/research/worawork-vercel-app-13882cbe/root-8a5edab2/NOTES.md`. Read it before touching this site's code — it documents deferred/known gaps that look like bugs but are deliberate (ponytail-flagged) simplifications.
...
- `assetPaths.ts` — every downloaded asset path, rooted at `/sites/worawork-vercel-app-13882cbe/root-8a5edab2`; `data/content.ts` — the real bio/experience/arts content extracted verbatim from the site.
...
- After editing `AGENTS.md`, run `bash scripts/sync-agent-rules.sh` to regenerate the platform-specific instruction files (`.cursor/rules`, `.windsurfrules`, `GEMINI.md`, `.amazonq/`, etc. all derive from it).
- After editing `.claude/skills/clone-website/SKILL.md`, run `node scripts/sync-skills.mjs` to regenerate the skill for every other platform (`.codex/skills`, `.cline/skills`, `.roo/skills`, `.kiro/skills`, command files under `.cursor/commands`, `.windsurf/workflows`, etc.).

Do not hand-edit the generated platform files directly — edit `AGENTS.md` / `SKILL.md` and re-run the sync script.
